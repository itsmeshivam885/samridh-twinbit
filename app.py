...
st.markdown("""
    ...
    .badge-verified { background-color: #DBEAFE; color: #1E40AF; border: 1px solid #BFDBFE; }
...
# ==============================================================================
    ...
    if not st.session_state.authenticated or st.session_state.user_role != "Field Officer":
        ...
        # PROPERLY RENDERED HTML TABLE FOR CLAIMS QUEUE
        table_rows = ""
        for claim in st.session_state.claims_db:
            ...
            table_rows += f"""
            <tr>
                <td><b>{claim['Claim_ID']}</b></td>
                <td>{claim['Farmer_Name']}</td>
                <td>{claim['Village']}</td>
                <td>{claim['Crop']}</td>
                <td>{claim['Loss_Type']}</td>
                <td>{claim['Reported_Damage_Pct']}%</td>
                <td><span class="badge-status {status_class}">{status_icon} {claim['Status']}</span></td>
                <td>{claim['AI_Confidence']}</td>
            </tr>
            """
        ...
        <table class="custom-table">
            <thead>
                <tr>
                    ...
                    <th>Status</th>
                    <th>AI Confidence & Anti-Fraud Score</th>
                </tr>
            </thead>
            <tbody>
                {table_rows}
        ...
        """
        st.markdown(queue_table_html, unsafe_allow_html=True)
